 ...
 */
 ...
 (function () {
    ...
    // console.log('getCookie',getCookie(' LOGIN_TOKEN_ADMIN'))
    var myToken = getCookie(' LOGIN_TOKEN_ADMIN')
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            function addFile(file) {
                let newfile = file.source.source
                let param = new FormData(); //创建form对象
				param.append('file', newfile, newfile.name);
                let req = new XMLHttpRequest();
                req.open("post", "http://manager.nnex.io/exchange_manager/common/notice-upload", false);
                req.setRequestHeader('authorization', 'token='+myToken);
                req.onreadystatechange = function () {
                    if (req.readyState === 4 && req.status === 200) {
                        let responseText = JSON.parse(req.responseText);
                        if(responseText.code == "100200") {
                            let addUrl = responseText.data.newFileName
                            let oldContent = editor.getContent()
                            editor.setContent(oldContent+`<img src='http://manager.nnex.io/notice/${addUrl}'><br/>`)
                        }
                    }
                };
                req.send(param);
